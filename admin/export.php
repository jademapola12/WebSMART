...
$top_most_viewed_query = $conn->query("SELECT title, SUM(views_count) AS view_count 
    FROM archive_list 
    WHERE status = '1'
    GROUP BY title
...
while ($row = $top_most_viewed_query->fetch_assoc()) {
    $output .= '<tr style="background-color:#ced4da;">';
...
$top_least_viewed_query = $conn->query("SELECT title, SUM(views_count) AS view_count 
    FROM archive_list 
    WHERE status = '1'
    GROUP BY title
...
while ($row = $top_least_viewed_query->fetch_assoc()) {
    $output .= '<tr style="background-color:#ced4da;">';
    $output .= '<td colspan="9">' . $row['title'] . '</td>';
    $output .= '<td align="center">' . $row['view_count'] . '</td>';
    $output .= '</tr>';
...
$top_most_searched_query = $conn->query("SELECT title, SUM(search_count) AS search_count 
    FROM archive_list 
    WHERE status = '1'
    GROUP BY title
...
while ($row = $top_most_searched_query->fetch_assoc()) {
    $output .= '<tr style="background-color:#ced4da;">';
    $output .= '<td colspan="9">' . $row['title'] . '</td>';
...
$top_least_searched_query = $conn->query("SELECT title, SUM(search_count) AS search_count 
    FROM archive_list 
    WHERE status = '1'
    GROUP BY title
...
while ($row = $top_least_searched_query->fetch_assoc()) {
    $output .= '<tr style="background-color:#ced4da;">';
    $output .= '<td colspan="9">' . $row['title'] . '</td>';
    $output .= '<td align="center">' . $row['search_count'] . '</td>';
    $output .= '</tr>';
